...
const PRIORITIES = { high: { label: 'High', color: 'text-red-500', bg: 'bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400' }, medium: { label: 'Medium', color: 'text-amber-500', bg: 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' }, low: { label: 'Low', color: 'text-blue-500', bg: 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' } };
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{counts.active} tasks remaining</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 mb-4 shadow-sm space-y-3">
          <input
            ...
            className="w-full text-sm border border-gray-200 dark:border-gray-700 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-gray-300"
          />
          <div className="flex gap-3 flex-wrap">
            <select value={form.priority} onChange={e => setForm(f => ({ ...f, priority: e.target.value as TodoItem['priority'] }))}
              className="text-sm border border-gray-200 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none">
      ...
      <div className="space-y-2">
        ...
        {filtered.map(todo => (
          <div key={todo.id} className={`bg-white dark:bg-gray-800 border rounded-xl px-4 py-3 flex items-start gap-3 group transition-all ${todo.completed ? 'opacity-60 border-gray-100 dark:border-gray-800' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 hover:shadow-sm'}`}>
            <input
              type="checkbox"
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-medium ${todo.completed ? 'line-through text-gray-400 dark:text-gray-600' : 'text-gray-800 dark:text-gray-100'}`}>{todo.text}</p>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                ...
                {todo.dueDate && (
                  <span className={`text-xs flex items-center gap-1 ${new Date(todo.dueDate) < new Date() && !todo.completed ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'}`}>
                    <Calendar size={10} />{todo.dueDate}
                  </span>
                )}
              </div>
            </div>
            <button onClick={() => remove(todo.id)} className="opacity-0 group-hover:opacity-100 text-gray-300 dark:text-gray-600 hover:text-red-500 transition-all flex-shrink-0">
              <X size={16} />
            </button>
          </div>
        ))}
...
function CheckmarkIcon() {
  return (
    <svg className="w-12 h-12 mx-auto text-gray-200 dark:text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
